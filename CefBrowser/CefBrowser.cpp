...
// CEFBrowser.cpp : 응용 프로그램에 대한 클래스 동작을 정의합니다.
//
...
BOOL CCefBrowserApp::PumpMessage()
{
	// do CEF message loop
...
int CCefBrowserApp::ExitInstance()
{
	...
	if( m_bCEFInitialized )
	{
		...
		m_cefApp = NULL;
		// shutdown CEF
		// 아래 꼭 호출해야 됨. 이유는   CefShutdown(); 호출해주어야 LocalStorage 또는 Cookies를 사용할 수 있음.
		CefShutdown();
	}
